...
async function fetchCharacter(name, status) {
    let url = baseUrl + "character";
    let response = await fetchData(url, name, status);
    let data = await response.text();
    return data;
}


async function fetchData(url, name, status) {
    try {
        const response = await fetch(url, { method: "GET", name: name, status: status});
        const result = await response.text();
        console.log(result);
        return response;
    } catch (error) {
...
let res = fetchCharacter('rick', 'alive');
console.log(res);
